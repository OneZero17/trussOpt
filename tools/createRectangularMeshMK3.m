function mesh = createRectangularMeshMK3(xMin, yMin, xMax, yMax, spacing)

xSpacingNumber = floor((xMax - xMin) / spacing);
ySpacingNumber = floor((yMax - yMin) / spacing);
...
mesh.Nodes(1, :) = xMin + mesh.Nodes(1, :) * ((xMax - xMin) / xSpacingNumber);
mesh.Nodes(2, :) = yMin + mesh.Nodes(2, :) * ((yMax - yMin) / ySpacingNumber);
